...
def process_fixture(match):
    ...
    try:
        ...
        for team_stats in stats_response:
            ...
            stats_list = team_stats.get('statistics', [])  # Fixed: comma was inside quotes
        ...
        logger.error(f"Error processing fixture {match.get('fixture', {}).get('id', 'unknown')}: {e}")
        return None, None
...
def process_fixtures(fixtures, processed_fixtures=None):
    ...
    with open(OUTPUT_FILE, mode, newline='', encoding='utf-8') as csvfile:
        ...
        for match in fixtures:
            fixture_unique_id, row_data = process_fixture(match)

            if not fixture_unique_id or not row_data:
                continue

            # Skip if already processed
            if fixture_unique_id in processed_fixtures:
...
def main():
    ...
    for season in SEASONS:
        ...
        # Fixed: Changed from process_fixture to process_fixtures
        season_processed = process_fixtures(fixtures, processed_fixtures)
...
# from src.utils import fetch_api_data, safe_get_value, format_date, logger
# # from Project_soccer.src.utils import fetch_api_data, safe_get_value, format_date, logger
