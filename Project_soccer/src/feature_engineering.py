...
def calculate_score_pattern_features():
    ...
    for season in seasons:  # Fixed: was 'season in season'
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        ...
        writer = csv.writer(f)  # Fixed: writer was not defined
...
def add_team_form_features(df, team_features):
    ...
    for team in team_features:
        for season in team_features[team]:
            ...
            goals_conceded = []  # Fixed: was using goals_scored list incorrectly
            for _, match in team_matches.iterrows():
                if match['Home Team'] == team:
                    goals_conceded.append(match['Goals Away'])
                else:
                    goals_conceded.append(match['Goals Home'])
            ...
            team_features[team][season]['Avg_Goals_Scored'] = sum(goals_scored) / len(goals_scored) if goals_scored else 0
            team_features[team][season]['Avg_Goals_Conceded'] = sum(goals_conceded) / len(goals_conceded) if goals_conceded else 0
            team_features[team][season]['Avg_xG'] = sum(xg_values) / len(xg_values) if xg_values else 0
